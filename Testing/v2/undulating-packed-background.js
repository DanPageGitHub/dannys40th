...
(() => {
  ...
  function makeSpiralPackedCircles(rand, baseRadius, minR, maxR, gap, targetCount) {
    ...
    const armCount = Math.floor(lerp(3, 6.999, rand()));
    const turns = lerp(2.6, 4.2, rand()) * lerp(0.9, 1.15, spiralBias);
    const direction = rand() > 0.5 ? 1 : -1;
    const armPhase = rand() * Math.PI * 2;
    const armJitter = lerp(0.04, 0.11, rand());
    const radialPower = lerp(0.78, 1.12, rand());
    const edgePad = gap * 1.4;
    const perArm = Math.round(targetCount * lerp(0.5, 0.7, rand()) / armCount);
    ...
    for (let arm = 0; arm < armCount; arm += 1) {
      ...
      for (let i = 0; i < perArm; i += 1) {
        ...
        const angle = armPhase
          + ((Math.PI * 2) / armCount) * arm
          + direction * turns * Math.PI * 2 * Math.pow(t, lerp(0.94, 1.06, rand()));
        const dist = baseRadius * 0.08 + (baseRadius * 0.8) * Math.pow(t, radialPower);
        const swirl = armJitter * baseRadius * (0.2 + t);
        const x = Math.cos(angle) * dist + Math.cos(angle * 3.1 + arm) * swirl * 0.28;
        const y = Math.sin(angle) * dist + Math.sin(angle * 2.7 + arm * 0.8) * swirl * 0.28;

        const ideal = lerp(maxR * 0.95, minR * 1.1, Math.pow(t, 0.9)) * lerp(0.82, 1.18, rand());
        let r = clamp(ideal, minR, maxR);

        if (previous) {
          const chainDistance = Math.hypot(x - previous.x, y - previous.y);
          r = Math.min(r, Math.max(minR, chainDistance - previous.r - gap));
        }

        const candidate = { x, y, r };
        if (r >= minR && insideCluster(candidate, baseRadius, edgePad) && !overlaps(candidate, placed, gap)) {
    ...
    const fillerTries = targetCount * 260;
    for (let i = 0; i < fillerTries && placed.length < targetCount; i += 1) {
      const t = rand();
      const arm = Math.floor(rand() * armCount);
      const angle = armPhase
        + ((Math.PI * 2) / armCount) * arm
        + direction * turns * Math.PI * 2 * Math.pow(t, lerp(0.85, 1.12, rand()))
        + (rand() - 0.5) * 0.55;
      const dist = baseRadius * 0.04 + (baseRadius * 0.88) * Math.pow(t, lerp(0.78, 1.08, rand()));
      const offset = lerp(-baseRadius * 0.11, baseRadius * 0.11, rand());
      const x = Math.cos(angle) * dist + Math.cos(angle + Math.PI / 2) * offset;
      const y = Math.sin(angle) * dist + Math.sin(angle + Math.PI / 2) * offset;
      const edge = baseRadius - Math.hypot(x, y);
      const allowed = clamp(edge - edgePad, minR, maxR);
      if (allowed < minR) continue;

      const radiusBias = Math.pow(rand(), lerp(1.8, 3.2, rand()));
      const r = lerp(minR, allowed, 1 - radiusBias);
      const candidate = { x, y, r };
      if (!insideCluster(candidate, baseRadius, edgePad)) continue;
      if (!overlaps(candidate, placed, gap)) placed.push(candidate);
    }
