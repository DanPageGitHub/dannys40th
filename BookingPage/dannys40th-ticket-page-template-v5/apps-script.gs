...
const FALLBACK_TALLY_SHEET_NAMES = ["Sheet1", "Danny's 40th Summer Weekender - 24th - 27th July"];
...
function doGet(e) {
  ...
  if (action === "checkTally") {
    ...
    return json({
      ...
      sheetName: match ? match.sheetName : "",
...
function findTallyByEmail(email) {
  ...
  const configured = spreadsheet.getSheetByName(TALLY_SHEET_NAME);
  if (configured) sheetsToTry.push(configured);

  FALLBACK_TALLY_SHEET_NAMES.forEach(name => {
    const s = spreadsheet.getSheetByName(name);
    if (s && !sheetsToTry.includes(s)) sheetsToTry.push(s);
  });

  const firstSheet = spreadsheet.getSheets()[0];
  if (firstSheet && !sheetsToTry.includes(firstSheet)) sheetsToTry.push(firstSheet);

  for (const sheet of sheetsToTry) {
    const match = findTallyByEmailInSheet(sheet, email);
...
function findTallyByEmailInSheet(sheet, email) {
  const lastRow = sheet.getLastRow();
  if (lastRow < 1) return null;

  const values = sheet.getRange(1, 1, lastRow, Math.max(2, sheet.getLastColumn())).getValues();
...
${clean.totalPayableToDanny > 0 ? "Payment link:\n" + clean.paymentLink + "\n\nPlease pay through Starling as soon as possible. Danny will manually match your payment against this booking." : "Nothing is payable to Danny through this form based on the options you selected."}
...
- Vans, campervans, motorhomes, electric hookup and glamping must be booked separately with the Barge Inn Honey Street.
